...
if($token!=null){
    ...
    if($cust_id!=null){
       ...
       // error_log(json_encode($data));
             ...
             $response["order"]=$cdb->insertIntoOrders($cust_id,$json_data);
             $response["error"] = false;
             $response["status"] = 1;
             $response["success"] = true;
             $response["message"] = "Payment Simulation was successful .   Your Order has been successfully submitted. You can view the order and track the process from  the my Orders menu";
