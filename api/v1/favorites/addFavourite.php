...
include '../classes/Favourites.php';
include '../classes/Products.php';
$pdb = new ProductsDbHandler;
$db = new DbHandler;
$fdb = new FavouritesDbHandler;
...
if($token!=null){
    ...
    if($cust_id!=null){
            ...
            $response["status"] = 1;
            $response["success"] = $fdb->favorite($cust_id,$_POST['productId']);
            $product=$pdb->product($_POST['productId']);

            //attachProductDetails
            //$response["product"] = $pdb->getProductRatingAndFavourite($product,$cust_id);
